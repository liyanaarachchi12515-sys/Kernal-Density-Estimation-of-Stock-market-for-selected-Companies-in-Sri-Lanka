...
data <- data_asian_alliance$High  # Using High as the example data

# Step 6: Define Bandwidth Grid Based on "Closing" Prices
data <- data_asian_alliance$Closing  # Using Closing prices as the example data
...
h_values <- seq(5,100 , by = 0.1)  # Bandwidth range to test
...
plot(h_values, cv_values, type = "b", col = "blue",
     xlab = "Bandwidth (h)", ylab = "CV(h)", main = "Cross-Validation for Bandwidth Selection Asian Alliance ")
...
xlab = "Data (High Prices)", ylab = "Density",
main = "Kernel Density Estimate (KDE) with Optimal Bandwidth Asian Alliance")
  
}


x_vals <- seq(min(data) - 2 * h, max(data) + 2 * h, length.out = 1000)
kde_values <- kde(x_vals, data, optimal_h, custom_kernel)
plot(x_vals, kde_values, type = "l", col = "blue", lwd = 2,
     xlab = "Data Range", ylab = "Density", main = "Kernel Density Estimate (KDE) Asian Alliance")


plot(h_values, cv_values, type = "b", col = "blue",
     xlab = "Bandwidth (h)", ylab = "CV(h)", main = "Cross-Validation for Bandwidth Selection Asian Alliance")
points(optimal_h, min(cv_values), col = "red", pch = 19)
